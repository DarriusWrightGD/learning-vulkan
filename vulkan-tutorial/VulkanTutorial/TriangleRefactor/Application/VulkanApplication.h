...
protected:
	virtual void CreateGraphicsPipeline(VkDevice device, VkPipelineLayout * pipelineLayout, VkPipeline * pipeline, VkExtent2D extent) = 0;
...
private:
	...
	void initVulkan() {
		...
		graphicsSystem->Initialize([this](const VkInstance & instance, VkSurfaceKHR * surface) { createSurface(instance, surface); },
			[this](VkDevice device, VkPipelineLayout * pipelineLayout, VkPipeline * pipeline, const VkExtent2D & extent) { return CreateGraphicsPipeline(device, pipelineLayout, pipeline, extent); },
